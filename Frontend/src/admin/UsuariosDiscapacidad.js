...
function UsuariosDiscapacidad() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/discapacidad')
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => setDiscapacidad(data))  // `setUsuarios` debería actualizar el estado con los datos
...
const filteredDiscapacidad = discapacidad
  .filter((discapacidad) => {
    ...
    const matchesSearchTerms = searchTerms.every((term) =>
      discapacidad.nombre.toLowerCase().includes(term) ||
      discapacidad.apellido_paterno.toLowerCase().includes(term) ||
      discapacidad.apellido_materno.toLowerCase().includes(term) ||
      discapacidad.fecha_nacimiento.toLowerCase().includes(term)
    );

    // Filtrar también por estado solo si la búsqueda se ha activado
    const matchesState = selectedState ? discapacidad.estado === selectedState : true;
 ...
 // Función para mostrar los documentos
  const getDocumentUrl = (docPath) => {
   ...
   const handleViewMore = (usuarioId) => {
    navigate(`/discapacidad/${usuarioId}`); // Redirige a la vista de detalles del usuario
  };
  ...
  <button 
    ...
    >
      ...
      <div className="container">
        <h2 className="section-title">Usuarios - Discapacidad</h2>

        {/* Datos Personales del Discapacidad */}
        <div className="section">
          <h3 className="subsection-title">Datos Personales de las Personas con Discapacidad</h3>
          <table className="data-table">
            ...
            <tbody>
              {filteredDiscapacidad.map((discapacidad) => (
                <tr key={discapacidad.beneficiario_id}>
                   
                  <td>{discapacidad.nombre}</td>
                  <td>{discapacidad.apellido_paterno}</td>
                  <td>{discapacidad.apellido_materno}</td>
                  <td>{discapacidad.edad}</td>
                  <td>{discapacidad.sexo}</td>
                  <td>{discapacidad.fecha_nacimiento ? new Date(discapacidad.fecha_nacimiento).toISOString().split('T')[0] : ''}</td>
                  <td>{discapacidad.curp}</td>
                  <td>{discapacidad.nivel_estudios}</td>
                  <td>
                    <button onClick={() => handleViewMore(discapacidad.beneficiario_id)}>Ver más</button>
                  </td>
                </tr>
              ))}
            </tbody>
