...
function UsuariosAdulto() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/adulto')
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => setAdulto(data))  // `setUsuarios` debería actualizar el estado con los datos
...
.filter((adulto) => {
  ...
  const matchesSearchTerms = searchTerms.every((term) =>
    adulto.nombre.toLowerCase().includes(term) ||
    adulto.apellido_paterno.toLowerCase().includes(term) ||
    adulto.apellido_materno.toLowerCase().includes(term) ||
    adulto.fecha_nacimiento.toLowerCase().includes(term)
  );

  // Filtrar también por estado solo si la búsqueda se ha activado
  const matchesState = selectedState ? adulto.estado === selectedState : true;
 ...
 // Función para mostrar los documentos
  const getDocumentUrl = (docPath) => {
   ...
   const handleViewMore = (usuarioId) => {
    navigate(`/adulto/${usuarioId}`); // Redirige a la vista de detalles del usuario
  };
  ...
  <button 
    ...
    >
      ...
      <div className="container">
        <h2 className="section-title">Usuarios - Tercera Edad</h2>

        {/* Datos Personales del adulto */}
        <div className="section">
          <h3 className="subsection-title">Datos Personales de las Personas de la Tercera Edad</h3>
          <table className="data-table">
            ...
            <tbody>
              {filteredAdulto.map((adulto) => (
                <tr key={adulto.beneficiario_id}>
                   
                  <td>{adulto.nombre}</td>
                  <td>{adulto.apellido_paterno}</td>
                  <td>{adulto.apellido_materno}</td>
                  <td>{adulto.edad}</td>
                  <td>{adulto.sexo}</td>
                  <td>{adulto.fecha_nacimiento ? new Date(adulto.fecha_nacimiento).toISOString().split('T')[0] : ''}</td>
                  <td>{adulto.curp}</td>
                  <td>{adulto.nivel_estudios}</td>
                  <td>
                    <button onClick={() => handleViewMore(adulto.beneficiario_id)}>Ver más</button>
                  </td>
                </tr>
              ))}
            </tbody>
