...
import './RegistroDiscapacidad';

function RegistroTerceraEdad() {
  ...
  const requiredFieldsByStep = {
    ...
    4: ["antecedentesPatologicos", "serviciosSalud", "descripcionApoyo"],
...
//terceraedad@gmail.com Moy1010@
  ...
  const handleChange = (e) => {
    ...
    formattedValue = value.replace(/[^0-9]/g, ''); // Solo números
    formattedValue = formattedValue.slice(0, 1); // Limitar a 2 caracteres
    } 
 ...
 // Maneja el cambio en el campo "Otro" solo al modificar el texto
  ...
  try {
    const response = await fetch('http://localhost:5000/api/adulto', {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        window.location.href = `/ProfileAdultos/${userId}`; 
